...
exports.getCategoria = async (req, res, next) => {
  ...
  try {
    var sql = ` select coalesce(max(ad.cod_categoria),0) + 1 as id 
                  from categoria ad 
...
exports.mainCategoria = async(req, res, next)=>{
  ...
  try {
    ...
    next()
    log_error.error({error, mensaje:'abm categoria'});
    console.log(error)
  }
}